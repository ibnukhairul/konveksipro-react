...
export default function QuantityModal({
  ...
  // Hitung total quantity dari ukuran
  const totalQuantity = Object.values(sizes).reduce((a, b) => a + b, 0)

  // Update harga berdasarkan total quantity
  useEffect(() => {
    if (productId && fabricId) {
  ...
  // Hitung subtotal dengan addon per pcs
  const calculateSubtotal = () => {
    ...
    // Hitung berdasarkan ukuran + addon per pcs
    let total = 0
    for (const [size, qty] of Object.entries(sizes)) {
      if (qty > 0) {
        const adjust = sizeAdjustments.find(s => s.size === size)?.additional_price || 0
        total += (pricePerUnit + adjust) * qty
      }
    }
    // Addon: per pcs dikalikan totalQuantity
    const addonsList = getAddonsForProduct(productId)
    for (const addonId of selectedAddons) {
      const addon = addonsList.find(a => a.id === addonId)
      if (addon) total += addon.price * totalQuantity
    }
    return total
  }
  ...
  const handleSubmit = () => {
    ...
    const selectedAddonDetails = selectedAddons.map(id => {
      ...
      return { ...a, pricePerPcs: a.price } // simpan harga per pcs
    })
    ...
    const item = {
      ...
      quantities: { ...sizes },
      addons: selectedAddonDetails.map(a => ({ ...a, price: a.pricePerPcs })), // simpan harga per pcs
    ...
    <div className="kpro-modal-overlay is-open" style={{ zIndex: 1000 }}>
      <div className="kpro-modal" style={{ maxWidth: '650px', maxHeight: '85vh', overflowY: 'auto' }}>
        ...
        <div className="kpro-modal-body">
            ...
            <div className="tier-list" style={{ marginBottom: '16px', background: '#F8FAFC', borderRadius: '12px', padding: '12px' }}>
              ...
              {priceInfo.minQty && totalQuantity < priceInfo.minQty && (
                <div style={{ marginTop: '8px', color: '#D97706', fontSize: '12px' }}>
                  ⚠️ Minimal order: {priceInfo.minQty} pcs. Harga yang digunakan adalah harga tier tertinggi.
                </div>
              )}
            </div>
          )}
          ...
          <div id="price-info" className="info-text" style={{ marginTop: '8px', padding: '8px', borderRadius: '8px', background: '#EFF6FF' }}>
            {!priceInfo.isNegotiable && !canNegotiate && (
              <div>💰 Harga: Rp {priceInfo.price.toLocaleString('id-ID')}/pcs <span style={{ fontSize: '12px' }}>({priceInfo.tierLabel})</span></div>
            )}
            {canNegotiate && (
              <div style={{ color: '#D97706' }}>⚠️ Harga untuk {totalQuantity} pcs dapat dinegosiasikan. Isi harga di bawah.</div>
            )}
          </div>
